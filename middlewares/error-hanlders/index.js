...
const errorHandler = (err, req, res, _next) => {
  ...
  } else if (err.name.toString().toUpperCase() === errors['400_EMPTY_FIELD']) {
    badRequestEmptyField(err, res);
  } else if (err.name.toString().toUpperCase() === errors['400_NOT_EXIST']) {
    badRequestNotExist(err, res);
  } else if (err.name.toString().toUpperCase() === errors['400_WRONG_FIELD']) {
  ...
  } else if (err.name.toString().toUpperCase() === errors['400_EXPIRED_OTP']) {
    res
      .status(400)
      .json(
